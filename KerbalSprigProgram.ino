...
void lightAction() { //This function toggles the light action group
  mySimpit.toggleAction(LIGHT_ACTION);
  sprigScreen.showStats("Lights toggled!");
}

void sasAction() { //This function toggles the SAS action group
  mySimpit.toggleAction(SAS_ACTION);
  sprigScreen.showStats("SAS toggled!");
}

void camYaw(int yawFactor) {//This function rotates the camera in a yaw motion
...
void camPitch(int pitchFactor) { //This function rotates the camera in a pitch motion
  camMsg.setPitch(pitchFactor);  
  mySimpit.send(CAMERA_ROTATION_MESSAGE, camMsg);
  camMsg.setPitch(0);
}

void thrustAction(int control) { //This function controls thrust control, 
...
void (*primaryActions[])() = { //Actions binded to the primary action section
  stageAction, sasAction, abortAction, lightAction, []() { //These are binded to W,A,S,D
    camPitch(10); //I
  },  
  []() {
    camYaw(10); //J
...
void (*secondaryActions[])() = { //Actions binded to the secondary action section (ie when the back button is pressed)
  []() {
    thrustAction(32767);
  },        // W
  nullptr,  // A (Unused)
...
void stageAction() {//This function activates the staging action group.
  mySimpit.activateAction(STAGE_ACTION);
  sprigScreen.showStats("Staged!");
}
